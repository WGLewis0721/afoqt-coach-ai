...
def generate_quiz_items(subtest: str, n: int, difficulty: str) -> list[dict[str, Any]] | None:
    ...
    prompt = (
        f"Generate {n} ORIGINAL AFOQT-style {subtest} items at {difficulty} difficulty. "
        "Follow QUIZ_SCHEMA.md. Return ONLY a JSON array of item objects with keys "
        "id, topic, stem, choices, answer, explanation, visual. Five choices A-E."
    )
    sys = system_prompt() + "\n\n" + context
    payload = {
...
def _fallback(user_text: str, context: str) -> str:
    ...
    if not q:
        return (
            ...
            "(llama3.1 recommended). Meanwhile use Quiz mode — the local item bank "
    ...
    return (
        ...
        f"You asked: {q}\n\n{excerpt}\n\n"
        "Start Ollama for full coaching. Quiz and readiness still work."
    )
